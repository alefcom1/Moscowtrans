...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const UPLOAD_ENDPOINT = (window.remarka_ajax && window.remarka_ajax.url) ? window.remarka_ajax.url : '/wp-admin/admin-ajax.php';
const UPLOAD_TOKEN    = 'rem-msc-2026';
const SITE_SOURCE     = 'Москва · moscowtrans.ru'; // ← то же, что в PHP

/**
 * Загружает массив File-объектов на сервер через WP AJAX.
...
async function uploadFilesToServer(files, onProgress) {
  ...
  fd.append('action', (window.remarka_ajax && window.remarka_ajax.action) ? window.remarka_ajax.action : 'remarka_upload');
  fd.append('nonce',  (window.remarka_ajax && window.remarka_ajax.nonce)  ? window.remarka_ajax.nonce  : '');
